...
if ( ! class_exists( 'Businessexpo_Bootstrap_Navwalker' ) ) :
	/**
	 * Businessexpo_Bootstrap_Navwalker class.
	 */
	class Businessexpo_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			 ...
			 * @param int      $depth   Depth of menu item. Used for padding.
			 */
			$class_names = join( ' ', apply_filters( 'businessexpo_nav_menu_submenu_css_class', $classes, $args, $depth ) );
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( $this->has_children && 0 === $depth ) {
				...
				$atts['class']         = 'dropdown-toggle nav-link chevron bottom';
				...
				if ( $depth > 0 ) {
					if ( $this->has_children && 1 === $depth ) {
						$atts['class'] = 'dropdown-item chevron-child bottom';
					} elseif ( $this->has_children && 2 === $depth ) {
						$atts['class'] = 'dropdown-item chevron-child bottom';
					} else {
			...
			$title = apply_filters( 'businessexpo_the_title', $item->title, $item->ID );
			 ...
			 * @param int              $depth Depth of menu item. Used for padding.
			 */
			$title = apply_filters( 'businessexpo_nav_menu_item_title', $title, $item, $args, $depth );
		...
		public static function fallback( $args ) {
			...
			if ( $args['container'] ) {
				 ...
				 */
				$allowed_tags = apply_filters( 'businessexpo_wp_nav_menu_container_allowedtags', array( 'div', 'nav' ) );
			...
			$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Add a menu', 'businessexpo' ) . '">' . esc_html__( 'Add a menu', 'businessexpo' ) . '</a></li>';
			$fallback_output .= '</ul>';
